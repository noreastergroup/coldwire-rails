...
function isAutoCacheable(request) {
  ...
  if (matchesRules(url, NEVER_CACHEABLE)) return false

  // A digested address is storable whatever else the app says. Precaching already stores
  // exactly these files by following a page's subresources, so this adds no new kind of thing
  // to the cache — it keeps what is there in step. Without it, viewing a page online replaces
  // the precached copy with a newer render and leaves its stylesheet behind at the old digest,
  // so the page in the cache asks offline for a file nobody stored.
  if (matchesRules(url, CACHE_FIRST)) return true

  if (CACHEABLE.length === 0) return true

  return matchesRules(url, CACHEABLE)
}
