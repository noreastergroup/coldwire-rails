...
// Deliberately narrow: this says nothing about what may be *stored*, only about whether a
// stored copy may answer while the network is up. Assets are cache-first however they are
// listed elsewhere, unless named here.
function mustRevalidate(request) {
  if (CACHE_REVALIDATE.length === 0) return false

  const url = new URL(request.url)
  if (url.origin !== self.location.origin) return false

  return matchesRules(url, CACHE_REVALIDATE)
}
