...
async function handleFetch(request) {
  ...
  // The app's own surfaces recache on every view so the copy stays fresh: a page, and equally
  // anything else the allowlist names, keeps its address while its contents change, so a
  // cached copy is only ever as good as its age. Assets and other origins' files stay
  // cache-first — their address changes with their contents, so refetching one gains nothing
  // and would bury the page in the inspector's recent list.
  if (cached && !wantsHtml(request) && !isOwnSurface(new URL(request.url))) return cached
