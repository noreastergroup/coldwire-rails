...
async function handleFetch(request) {
  ...
  // Pages recache on every view so the copy stays fresh; everything else is cache-first, or
  // every stylesheet would bury the page in the inspector's recent list. An address that
  // outlives its contents is the exception, and only where the app has said so: a cached copy
  // of one is only ever as good as its age.
  if (cached && !wantsHtml(request) && !mustRevalidate(request)) return cached
