...
async function runSync() {
  ...
  for (const href of wanted) {
    // The page, not a frame of it. A frame entry answering for the page here would mark a
    // manifest URL as already held, and the page itself would never be fetched.
    const held = await cache.keys(new Request(href), MATCH_OPTIONS)
    const key = held.find((candidate) => variantOf(candidate) === "page")
