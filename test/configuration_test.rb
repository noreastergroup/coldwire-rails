...
class ConfigurationTest < Minitest::Test
  ...
  def test_hosts_are_bare
    config.cacheable_hosts = [ "tiles.example.com", "localhost:3001" ]

    assert_equal [ "tiles.example.com", "localhost:3001" ], config.cacheable_hosts
  end

  # A scheme is what everybody will type, having pasted a URL — and it is exactly the thing
  # that changed, so it is told rather than quietly fixed. Stripping it would leave a config
  # that looks migrated and is not.
  def test_a_scheme_is_refused_and_the_error_says_what_to_write
    error = assert_raises(ArgumentError) { config.cacheable_hosts = [ "https://tiles.example.com" ] }

    assert_match(/no scheme/, error.message)
    assert_match(/"tiles\.example\.com"/, error.message)
  end

  def test_case_does_not_matter
    config.cacheable_hosts = [ "Tiles.Example.COM" ]

    assert_equal [ "tiles.example.com" ], config.cacheable_hosts
  end
  ...
  def test_a_path_is_refused_rather_than_silently_ignored
    error = assert_raises(ArgumentError) { config.cacheable_hosts = [ "tiles.example.com/maps" ] }

    assert_match(/bare hosts/, error.message)
  end

  def test_nonsense_is_refused
    assert_raises(ArgumentError) { config.cacheable_hosts = [ "not a domain" ] }
    assert_raises(ArgumentError) { config.cacheable_hosts = [ "" ] }
  end
end
